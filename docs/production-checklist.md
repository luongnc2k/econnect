...
- [ ] `INTERNAL_JOB_RUNNER_ENABLED` và `INTERNAL_JOB_RUNNER_INTERVAL_SECONDS` đã được chốt đúng theo mô hình vận hành
- [ ] `CANCEL_UNDERFILLED_CLASSES_HOURS` đã được chốt đúng theo rule hủy lớp thiếu học viên của production
