...
- nếu `refund_payout` bị lỗi, admin có thể gọi `POST /payments/bookings/{booking_id}/retry-refund`
...
- nếu lớp đã kết thúc nhưng không còn học viên hợp lệ nào, backend sẽ auto-cancel lớp và hoàn phí tạo lớp cho Tutor ngay khi job `release-eligible-payouts` chạy
